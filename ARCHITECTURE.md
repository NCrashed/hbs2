...
wire-format specification (see [`PROTOCOL.md`](PROTOCOL.md), to come).
...
| `hbs2-storage-simple` | On-disk storage backend. NCQ3 log-structured format. Implements the storage interface from `hbs2-core`. |
...
| `hbs2-keyman` | Key manager service. Tracks where local private keys live and serves them to local clients that need to sign or decrypt. |
| `hbs2-cli` | Command-line interface that talks to a running `hbs2-peer` over its RPC socket. Used for inspecting refchans, reflogs, storage, peers, and so on. |
| `hbs2-git` | Git remote helper. Provides the `hbs2://` protocol for git, plus `git hbs2 export` and `hbs2-git-subscribe` commands. |
...
| `hbs2` | Small umbrella executable that brings together `hbs2-core`, `hbs2-storage-simple`, `hbs2-peer` libraries and exposes a handful of top-level commands like `keyring-new`. |
...
1. Git invokes `git-remote-hbs2` (a binary from `hbs2-git`). This is
   a standard "remote helper": git speaks a documented stdin/stdout
   protocol to it.
2. `git-remote-hbs2` connects to the local `hbs2-peer` over its RPC
...
- **`RefLog`**. An append-only log of signed transactions. One
  ...
  root of the repository state). Used by `hbs2-git`.
...
into a keyring file by `hbs2 keyring-new`. The keyring file contains
both halves; `hbs2 keyring-list` extracts the public half. The
private half is used to sign block announcements and to participate
in protocols that authenticate the peer.
...
`hbs2-storage-simple` implements the `Storage` interface from
`hbs2-core/lib/HBS2/Storage.hs`. The on-disk format is **NCQ3**, a
log-structured immutable store. Blocks are appended to segment files
and indexed; obsolete data is reclaimed by rewriting segments. The
implementation has gone through three iterations (NCQv1, NCQv2,
NCQ3); only NCQ3 is supported.
...
- **Unix sockets**. Used for the local RPC between `hbs2-peer` and
  client tools (`hbs2-cli`, `git-remote-hbs2`, `hbs2-sync`,
...
- `bootstrap-dns <domain>` looks up TXT records at that domain and
  treats their content as peer addresses. The legacy
  `bootstrap.hbs2.net` is no longer served; bring your own.
...
```
   ...
   |         |       |            |        |         |
   v         v       v            v        v         v
 hbs2-cli  git-     hbs2-sync   hbs2-      hbs2     (any other
           remote-              keyman     (CLI       client)
           hbs2                            shims)
```
...
- **The legacy test suite is partial.** `hbs2-tests` has working
  tests and bit-rotted tests side by side. Two suites are explicitly
  disabled with `buildable: False` and need a rewrite against the
  current `hbs2-core` API.

- **Storage rewrite is in flight.** Pre-NCQ3 layouts are gone, but
  a further move toward sstable-style LSM (for scaling beyond
  hundreds of thousands of objects) was planned in the legacy
  roadmap and not done. Worth picking up if you care about
...
- `hbs2-peer/app/PeerMain.hs` is the daemon's main. Big file (~1300
  lines) but follows a clear "wire up all the things and run" shape.
- `hbs2-git/lib/` has the git remote helper logic. The interface to
  `hbs2-peer` is via the storage and reflog RPC APIs.
...
- `hbs2-core/lib/HBS2/Net/Proto/` has the protocol type machinery.
  Start with `Types.hs`.
- `hbs2-storage-simple/lib/` has the NCQ3 implementation.

For protocol-level detail beyond this overview, see the planned
[`PROTOCOL.md`](PROTOCOL.md). The legacy `docs/` directory contains
older design notes that may also be useful, in Russian and English
mixed.
