...
\textbf{Историческая справка.} Этот документ добавлен коммитом
\texttt{5c2d7f92} 26.06.2023 и с тех пор не менялся. Он описывает hbs2-git
времён 0.24.x и сохранён как запись того дизайна, а не как описание
работающей системы.

Работающая реализация называется hbs2-git3 и расходится с написанным ниже в
трёх местах:

\begin{itemize}
  \item Исполняемые файлы теперь \texttt{hbs2-git3} и
        \texttt{git-remote-hbs23}. Боксов \texttt{git-hbs2} и
        \texttt{git-remote-hbs2} с диаграммы в разделе <<Компоненты>>
        больше не существует, как и пакета \texttt{hbs2-git}.
  \item hbs2-git3 разговаривает с hbs2-peer только по RPC через unix-сокет
        (\texttt{StorageAPI}, \texttt{RefLogAPI}, \texttt{LWWRefAPI},
        \texttt{PeerAPI}) и не использует HTTP вообще, так что все три
        стрелки <<HTTP: \ldots>> на той же диаграмме относятся только к
        0.24.x. HTTP-интерфейс пира с тех пор стал доступен только на
        чтение и по умолчанию слушает 127.0.0.1: эндпойнта \texttt{PUT /},
        который стоит за стрелкой <<HTTP: Put>>, больше нет.
  \item Формат другой. Раздел про сегментированный лог (секции $S_n$,
        \texttt{HEAD(64)}, типы \texttt{GitLogEntry*}, запись
        \texttt{RepoHead}) и раздел про стейт в SQLite описывают 0.24.x. В
        hbs2-git3 транзакция рефлога это сегмент либо чекпойнт с рангом.
\end{itemize}
