...
import HBS2.Hub.CLI.Dict (hubEntries,ours,peerFulNames)
...
main = do
  ...
  -- THE SURFACE IS AN ALLOWLIST and it lives in 'HBS2.Hub.CLI.Dict', with the
  -- verbs it admits and the reasoning behind both. Here because @--run@
  -- evaluates in THIS dictionary: a script gets the forge verbs in sequence and
  -- nothing that touches the filesystem or spawns a process.
  let dict = HM.filterWithKey (\k _ -> ours k) $ makeDict do
        internalEntries
        hubEntries
